...
export default function EditarLifeGroupPage() {
  ...
  useEffect(() => {
    async function carregarGrupo() {
      ...
      if (error || !grupo) {
        setErro("Life group não encontrado.");
        setCarregando(false);
        return;
      }

      setNome(grupo.nome);
      setLider(grupo.lider);
      setBairro(grupo.bairro ?? "");
      setEndereco(grupo.endereco ?? "");
      setDiaSemana(String(grupo.dia_semana ?? "0"));
      setHorario(grupo.horario ?? "");
      setCapacidade(grupo.capacidade ? String(grupo.capacidade) : "");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (error) {
      setErro("Erro ao atualizar life group: " + error.message);
      return;
    }

    alert("Life group atualizado com sucesso!");
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-4 py-10">
      <div className="max-w-2xl mx-auto">
        ...
        <h1 className="text-2xl font-bold mt-2 mb-8">Editar Life Group</h1>
